...
public class D0_CategoryFragment extends BaseFragment implements
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
			...
			public boolean onEditorAction(TextView v, int actionId,
				...
				if (actionId == EditorInfo.IME_ACTION_SEARCH) {
					try {
						...
						intent.putExtra(B1_ProductListActivity.FILTER, filter
								.toJson().toString());
		...
		parentListAdapter = new D0_CategoryAdapter(getActivity(),
	...
	public void showRecognizerDialog() {
		...
		if (appid != null && !"".equals(appid)) {
			...
			SpeechUser.getUser().login(getActivity(), null, null,
					...
					listener);
			final RecognizerDialog recognizerDialog = new RecognizerDialog(
					getActivity());
			// 设置引擎为转写
	...
	private SpeechListener listener = new SpeechListener() {
		...
		public void onCompleted(SpeechError error) {
			if (error != null) {
				Toast.makeText(getActivity(), "登陆失败", Toast.LENGTH_SHORT)
						.show();
